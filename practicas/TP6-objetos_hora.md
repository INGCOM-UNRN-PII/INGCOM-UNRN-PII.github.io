---
title: "TP6 - Clase Hora (Tiempo Sexagesimal)"
description: Ejercicio de implementación de clase Hora para manejo de tiempo.
---

# Clase Hora - Tiempo Sexagesimal

Diseñá e implementá en Java una clase llamada `Hora` que represente y manipule unidades de tiempo en formato
sexagesimal (horas, minutos, segundos).

## Representación Interna

- Utilizá tres atributos privados de tipo `short` para almacenar los minutos y los segundos, con las horas como tipo `int`.
...
Implementá un método `compareTo(Hora otraHora)` que compare la instancia actual de `Hora` con otra instancia (`otraHora`). El método debe retornar:
...
significativa de objetos `Hora` y su uso en colecciones basadas en hash (como `HashSet` o `HashMap`).
...
- La correcta implementación de `equals()` y `hashCode()`, verificando la igualdad de objetos con los mismos valores
  y la desigualdad con valores diferentes. También probá la consistencia entre `equals()` y `hashCode()`.
:::
