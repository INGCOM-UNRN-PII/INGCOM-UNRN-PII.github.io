...
description: Convenciones de código para el lenguaje de programación Java, traducidas al español.
---
...
- **Traducido al castellano**: 10 de Mayo del 2001 por Alberto Molpeceres ([javaHispano](http://www.javahispano.com))
- **Revisión y corrección**: Marzo de 2007 por Lucas Vieites ([Codexion](http://www.codexion.com))
:::
...
Un archivo está formado por secciones que deben estar separadas por líneas en blanco y comentarios opcionales que identifican cada sección. Se deberá evitar la creación de archivos de más de 2000 líneas puesto que son incómodos de manejar. Para ver un ejemplo de un programa de Java debidamente formateado, vea [Ejemplo de archivo fuente Java](#ejemplo-archivo-fuente).
...
:::{note}
El primer componente del nombre de un paquete único se escribe siempre en minúsculas con caracteres ASCII y debe ser uno de los nombres de dominio de último nivel, actualmente `com`, `edu`, `gov`, `mil`, `net`, `org`, o uno de los códigos de dos letras que especifican el país como se define en el estándar ISO 3166, 1981.
:::

#### 3.1.3 Declaraciones de clases e interfaces

La siguiente tabla describe las partes de la declaración de una clase o interfaz, en el orden en el que deberían aparecer. Vea [Ejemplo de archivo fuente Java](#ejemplo-archivo-fuente).
...
:::{note}
Los ejemplos destinados a uso en la documentación deben tener una longitud inferior, generalmente no más de 70 caracteres.
:::
...
:::{tip}
La frecuencia de comentarios a veces refleja la escasez de calidad del código. Cuando se sienta obligado a escribir un comentario considere reescribir el código para hacerlo más claro.
:::
...
:::{note}
Si no se va a utilizar `indent(1)`, no se deberá usar `/*-` en el código o hacer ninguna otra concesión a la posibilidad de que alguien ejecute `indent(1)` sobre él.
:::
...
:::{note}
Vea [Ejemplo de archivo fuente Java](#ejemplo-archivo-fuente) para obtener ejemplos de los formatos de comentarios descritos aquí. Para más detalles, vea [How to Write Doc Comments for Javadoc](http://java.sun.com/products/jdk/javadoc/writingdoccomments.shtml).
:::
...
:::{warning}
No ponga diferentes tipos en la misma línea:
...
:::{note}
Los ejemplos anteriores usan un espacio entre el tipo y el identificador. Una alternativa aceptable es usar tabuladores:
...
:::{warning}
Evite las declaraciones locales que oculten declaraciones de niveles superiores. No declare el mismo nombre de variable en un bloque interno:
...
:::{warning}
Las sentencias `if` usan siempre llaves `{}`. Evite la siguiente forma, propensa a errores:
...
:::{tip}
Al usar el operador «coma» en la cláusula de inicialización o actualización de una sentencia `for` evite la complejidad de usar más de tres variables. Si es necesario utilice sentencias separadas antes del lazo `for` (para la cláusula de inicialización) o al final del lazo (para la cláusula de actualización).
:::
...
:::{important}
- Cada vez que un caso se propaga (no incluye la sentencia `break`), añada un comentario donde normalmente se encontraría la sentencia `break`.
...
- **Palabra clave seguida de paréntesis:** Una palabra clave del lenguaje seguida por un paréntesis debe separarse por un espacio:
...
:::{note}
No se debe usar un espacio en blanco entre el nombre de un método y su paréntesis de apertura. Esto ayuda a distinguir palabras clave de llamadas a métodos.
:::

- **Después de comas:** Debe aparecer un espacio en blanco después de cada coma en las listas de argumentos.

- **Operadores binarios:** Todos los operadores binarios excepto `.` se deben separar de sus operandos con espacios en blanco. Los espacios en blanco no deben separar los operadores unarios, incremento (`++`) y decremento (`--`) de sus operandos:
...
- **Expresiones en sentencias for:** Las expresiones en una sentencia `for` se deben separar con espacios en blanco:
...
El siguiente ejemplo muestra el formato de archivo Java con una sola clase pública. Las interfaces se formatean de modo similar. Vea [Declaraciones de clases e interfaces](#64-declaraciones-de-clases-e-interfaces) y [Comentarios de documentación](#comentarios-de-documentación).
...
Al copiar, adaptar o redistribuir este documento siguiendo las indicaciones anteriores, está obligado a conceder el crédito a Sun. Si reproduce o distribuye el documento sin ninguna modificación sustancial en su contenido, use la siguiente línea de crédito:

> Copyright 1995-1999 Sun Microsystems, Inc. All rights reserved. Used by permission.

Si modifica este documento de forma que altera su significado, por ejemplo, para seguir las convenciones propias de su empresa, use la siguiente línea de créditos:
...
Se puede y se debe aplicar a esta traducción las mismas condiciones de licencia que al original de Sun en lo referente a uso, modificación y redistribución. Si distribuyes esta traducción (aunque sea con otro formato de estilo) estás obligado a dar créditos a javaHispano por la traducción indicándolo con las siguientes líneas:
