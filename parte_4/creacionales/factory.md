...
```{exercise}
:label: ej-factory-notificacion

Crea un factory para crear notificaciones:
1. Interfaz `Notificacion` con `enviar(String mensaje)`
2. Implementaciones: `NotificacionEmail`, `NotificacionSMS`, `NotificacionPush`
3. `NotificacionFactory` que cree instancias según tipo
4. Cada notificación tiene configuración específica
```

```{solution} ej-factory-notificacion
:class: dropdown
...
public class NotificacionFactory {
    public static Notificacion crear(String tipo, String destino) {
        switch (tipo) {
            case "EMAIL":
                return new NotificacionEmail(destino);
            case "SMS":
                return new NotificacionSMS(destino);
            default:
...
// Uso:
Notificacion email = NotificacionFactory.crear("EMAIL", "usuario@example.com");
email.enviar("¡Hola!");

Notificacion sms = NotificacionFactory.crear("SMS", "+541234567890");
sms.enviar("¡Hola!");
```
```
