...
```{exercise}
:label: ej-builder-sql

Crea un `QueryBuilder` que construya consultas SQL paso a paso:
1. Métodos: `select()`, `from()`, `where()`, `orderBy()`, `limit()`
2. Retorne la consulta construida con `construir()`
3. Valide que select() y from() sean obligatorios
```

```{solution} ej-builder-sql
:class: dropdown
...
public class QueryBuilder {
    ...
    public QueryBuilder select(String columnas) {
        this.selectClause = "SELECT " + columnas;
        return this;
    }
    
    public QueryBuilder from(String tabla) {
        this.fromClause = "FROM " + tabla;
        return this;
    }
    
    public QueryBuilder where(String condicion) {
        this.whereClause = "WHERE " + condicion;
        return this;
    }
    
    public QueryBuilder orderBy(String columna) {
        this.orderByClause = "ORDER BY " + columna;
        return this;
    }
    
    public QueryBuilder limit(int cantidad) {
        this.limitValue = cantidad;
        return this;
    }
    
    public String construir() {
        if (selectClause == null || fromClause == null) {
            throw new IllegalStateException("SELECT y FROM son requeridos");
        }
        ...
        if (orderByClause != null) {
            sql.append(orderByClause).append(" ");
        }
        if (limitValue != null) {
